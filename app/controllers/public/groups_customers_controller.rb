class Public::GroupsCustomersController < ApplicationController
	...
	def create
		...
		if @group_customer.group.is_closed == false
			...
			redirect_to maps_public_groups_customer_path(@group_customer.group.id) and return
		else
			...
			# key_word(合言葉)の間違い
				flash[:not_key_word] = "合言葉が間違っています"
      			redirect_to key_word_public_groups_customer_path(@group_customer.group.id) and return
			end
		end
	end
